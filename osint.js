...
GI.classify = function classify(input) {
    ...
    // domain
    if (/^([a-z0-9-]+\.)+[a-z]{2,}$/i.test(q)) return { kind: "domain", q: q.toLowerCase() };
    // username fallback
...
GI.doh = async function doh(name, type = "A") {
    ...
    try {
        ...
        if (r.ok) {
            ...
            return (j.Answer || []).map(a => ({ name: a.name, type: a.type, data: a.data, ttl: a.TTL }));
        }
    } catch { /* fallthrough */ }
    // Fallback to Google
    try {
        const r = await xfetch(`https://dns.google/resolve?name=${encodeURIComponent(name)}&type=${type}`);
        if (r.ok) {
            const j = await r.json();
            return (j.Answer || []).map(a => ({ name: a.name, type: a.type, data: a.data, ttl: a.TTL }));
        }
...
GI.certs = async function certs(domain) {
    try {
        const r = await xfetch(`https://crt.sh/?q=%25.${encodeURIComponent(domain)}&output=json`);
...
GI.rdap = async function rdap(target, kind) {
    ...
    try {
        const r = await xfetch(`https://rdap.org/${path}/${encodeURIComponent(target)}`);
...
GI.shodan = async function shodan(ip) {
    try {
        const r = await xfetch(`https://internetdb.shodan.io/${encodeURIComponent(ip)}`);
        if (r.status === 404) return { ip, ports: [], cpes: [], hostnames: [], tags: [], vulns: [] };
...
GI.urlscan = async function urlscan(query) {
    try {
        const r = await xfetch(`https://urlscan.io/api/v1/search/?q=${encodeURIComponent(query)}&size=20`);
...
GI.wayback = async function wayback(url) {
    try {
        const r = await xfetch(`https://archive.org/wayback/available?url=${encodeURIComponent(url)}`);
...
GI.github = async function github(user) {
    try {
        const [userRes, repoRes] = await Promise.all([
            ...
            xfetch(`https://api.github.com/users/${encodeURIComponent(user)}/repos?per_page=30&sort=updated`),
        ]);
...
async function md5(str) {
    ...
    function K(G, k) {
        let I, d, F, H, x; F = (G & 2147483648); H = (k & 2147483648);
    ...
    function e(G) {
        ...
        Z = (H - (H % 4)) / 4; d = (H % 4) * 8; aa[Z] = aa[Z] | (0x80 << d); aa[I - 2] = F << 3; aa[I - 1] = F >>> 29; return aa;
    }
    function B(x) { let k = "", F = "", G, d; for (d = 0; d <= 3; d++) { G = (x >>> (d * 8)) & 255; F = "0" + G.toString(16); k = k + F.substr(F.length - 2, 2); } return k; }
    ...
    let C = [], P, h, E, v, g, Y, X, W, V, S = 7, Q = 12, N = 17, M = 22, A = 5, z = 9, y = 14, w = 20, o = 4, m = 11, l = 16, j = 23, U = 6, T = 10, R = 15, O = 21;
    str = J(str); C = e(str); Y = 1732584193; X = 4023233417; W = 2562383102; V = 271733878;
...
GI.email = async function email(addr) {
    ...
    const DISPOSABLE = ["mailinator.com", "guerrillamail.com", "10minutemail.com", "trashmail.com",
        "yopmail.com", "tempmail.com", "dispostable.com", "maildrop.cc", "getnada.com",
        "sharklasers.com", "fakeinbox.com", "tempr.email", "throwawaymail.com", "mohmal.com"];
...
GI.pwnedPassword = async function pwnedPassword(password) {
    ...
    for (const line of text.split(/\r?\n/)) {
        ...
        if (s && s.trim() === suffix) return { pwned: true, count: parseInt(c, 10) };
    }
...
GI.hibpBreaches = async function hibpBreaches(email, apiKey) {
    ...
    try {
        const r = await xfetch(`https://haveibeenpwned.com/api/v3/breachedaccount/${encodeURIComponent(email)}?truncateResponse=false`, {
...
async function loadWMN() {
    ...
    try {
        const r = await xfetch("https://raw.githubusercontent.com/WebBreacher/WhatsMyName/main/wmn-data.json");
...
GI.usernameLinks = async function usernameLinks(username) {
    ...
    const sites = (data.sites || []).map(s => ({
        name: s.name,
        category: s.cat,
        url: s.uri_pretty ? s.uri_pretty.replace("{account}", username) : s.uri_check.replace("{account}", safe),
 ...
 */
GI.usernameProbe = async function usernameProbe(username, onResult, { limit = 50 } = {}) {
    const sites = await GI.usernameLinks(username);
    const slice = sites.slice(0, limit);
    const results = [];
    await Promise.all(slice.map(async s => {
        try {
            const r = await xfetch(s.check_url, { method: "GET", mode: "no-cors" }, 6000);
            // no-cors → opaque; we infer "reachable" from promise fulfilment only
            const hit = { site: s.name, url: s.url, check: s.check_url, status: "reachable", category: s.category };
            results.push(hit);
            if (onResult) onResult(hit);
        } catch {
            const hit = { site: s.name, url: s.url, check: s.check_url, status: "blocked", category: s.category };
            results.push(hit);
            if (onResult) onResult(hit);
        }
    }));
...
GI.dossier = async function dossier(input, { hibpKey } = {}) {
    ...
    const tasks = [];
    const add = (name, p) => tasks.push(p.then(v => { report.modules[name] = v; _feed("ok", `DOSSIER :: ${name} ✓`); })
        .catch(e => { report.modules[name] = { error: e.message }; _feed("err", `DOSSIER :: ${name} → ${e.message}`); }));

    if (cls.kind === "ipv4" || cls.kind === "ipv6") {
        add("geoip", window.fetchGeo ? window.fetchGeo(cls.q) : Promise.resolve(null));
        ...
        if (a.length) {
            ...
            add("geoip", window.fetchGeo ? window.fetchGeo(a[0].data) : Promise.resolve(null));
        }
...
GI.satellites = {
    ...
    async boot() {
        ...
        if (!viewer) return;
        const groups = [
            { url: "https://celestrak.org/NORAD/elements/gp.php?GROUP=stations&FORMAT=tle", color: "#12ffc6", label: true },
            { url: "https://celestrak.org/NORAD/elements/gp.php?GROUP=starlink&FORMAT=tle&LIMIT=60", color: "#ff2e6e", label: false, limit: 60 },
            { url: "https://celestrak.org/NORAD/elements/gp.php?GROUP=gps-ops&FORMAT=tle", color: "#ffb020", label: false },
        ];
        for (const g of groups) {
            try {
                const r = await xfetch(g.url, {}, 15000);
                if (!r.ok) { _feed("err", `SAT :: fetch ${g.url} → HTTP ${r.status}`); continue; }
                const text = await r.text();
                ...
                _feed("ok", `SAT :: spawned ${spawned} from ${g.url.split("GROUP=")[1].split("&")[0]}`);
            } catch (e) {
    ...
    clear() {
        const viewer = _viewer();
        if (!viewer) return;
        this._added.forEach(id => { try { viewer.entities.removeById(id); } catch { } });
        this._added = [];
    }
};
...
GI.exportMarkdown = function exportMarkdown(report, filename = "gideon-dossier.md") {
    const L = [];
...
GI.replay = {
    ...
    all() { return this._hits.slice(); },
    clear() { this._hits = []; localStorage.removeItem("gi:hits"); },
    async play(speedMs = 400) {
        for (const h of this._hits) {
            if (h.lat != null && h.lon != null && _pin) {
                _pin({ label: h.label || h.slug, lat: h.lat, lon: h.lon, meta: h.tag, danger: true });
    ...
    try {
        ...
        if (t) {
            ...
            if (hops.length && t) {
                ...
                hops.push({ label: `ASN${(bgp && bgp.prefixes && bgp.prefixes[0] && bgp.prefixes[0].asn && bgp.prefixes[0].asn.asn) || "?"}`, lat: midLat, lon: midLon });
            }
            ...
            _viewer().entities.add({
                polyline: {
                    ...
                    material: new Cesium.PolylineGlowMaterialProperty({
                        glowPower: 0.3, color: Cesium.Color.fromCssColorString("#ffb020").withAlpha(0.85),
                    }),
                    arcType: Cesium.ArcType.NONE,
                }
            });
        }
        if (_pin) _pin({ label: a.label, lat: a.lat, lon: a.lon, meta: `hop ${i}`, danger: i === hops.length - 2 });
    }
    if (hops.length) _pin({ label: hops[hops.length - 1].label, lat: hops[hops.length - 1].lat, lon: hops[hops.length - 1].lon, meta: "TARGET", danger: true });
    return hops;
};
...
GI.renderKVs = function renderKVs(container, obj, depth = 0) {
    if (!container) return;
    if (obj == null) { container.innerHTML = `<div class="placeholder">—</div>`; return; }
    if (typeof obj !== "object") { container.textContent = String(obj); return; }
    const rows = [];
    for (const [k, v] of Object.entries(obj)) {
        if (v == null) continue;
        if (typeof v === "object") {
            const label = Array.isArray(v) ? `${v.length} item${v.length === 1 ? "" : "s"}` : "{...}";
            rows.push(`<details class="kv-group"><summary><span class="k">${k}</span> <span class="v">${label}</span></summary><pre class="kv-json">${escapeHtml(JSON.stringify(v, null, 2))}</pre></details>`);
        } else {
            rows.push(`<div class="kv"><span class="k">${k}</span><span class="v">${escapeHtml(String(v))}</span></div>`);
        }
    }
...
// Signal ready
console.log("%cGideonIntel OSINT engine v2 ready — window.GI", "color:#12ffc6; font-weight:bold");
