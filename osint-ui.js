...
(function () {
    const GI = window.GI;
    if (!GI) { console.error("GI engine missing"); return; }
    ...
    if (hibpKeyEl) {
        ...
        hibpKeyEl.addEventListener("change", () => localStorage.setItem("gi:hibp", hibpKeyEl.value.trim()));
    }
    ...
    async function runSweep() {
        ...
        const report = await GI.dossier(q, { hibpKey: (localStorage.getItem("gi:hibp") || "").trim() });
        lastReport = report;
        renderReport(report);
        ...
        if (geo && geo.lat && geo.lon && window.pinTarget) {
            window.pinTarget({
                label: q, lat: geo.lat, lon: geo.lon,
    ...
    function renderReport(report) {
        const parts = [];
        parts.push(`<div class="kv"><span class="k">DURATION</span><span class="v">${report.finishedMs} ms</span></div>`);
    ...
    function summary(mod, d) {
        ...
        switch (mod) {
            case "geoip": return `${d.city || "?"}, ${d.country_code || "?"}`;
            case "shodan": return `${(d.ports || []).length} port · ${(d.vulns || []).length} CVE`;
            case "bgp": return `${d.prefixes ? d.prefixes.length + " prefix" : (d.name || "asn")}`;
            case "rdap": return d.handle || d.name || d.ldhName || "ok";
            case "dns": return Object.entries(d).filter(([, v]) => Array.isArray(v) && v.length).map(([k]) => k).join(" · ");
            case "certs": return `${(d.subs || []).length} subdomains`;
            case "urlscan": return `${d.total || 0} scans`;
            case "wayback": return d.archived_snapshots && d.archived_snapshots.closest ? "archived" : "no snapshot";
            case "email": return d.gravatar_exists ? "gravatar ✓" : (d.disposable ? "disposable" : "ok");
            case "username": return `${(d || []).length} sites`;
            case "github": return d.user ? `@${d.user.login} · ${d.user.public_repos} repos` : "none";
            case "hibp": return `${(d.breaches || []).length} breaches`;
            default: return "ok";
        }
    }
    ...
    function renderModule(mod, d) {
        ...
        if (d.error) return `<div class="placeholder" style="color:var(--danger)">error: ${escapeHtml(d.error)}</div>`;
        switch (mod) {
            case "geoip": return kvList({
                IP: d.ip, CITY: d.city, REGION: d.region,
                COUNTRY: `${d.country || ""} (${d.country_code || ""})`,
                LAT: d.lat, LON: d.lon, ORG: d.org, ASN: d.asn, TZ: d.timezone,
            });
            case "shodan":
                return [
                    kvList({
            ...
            case "bgp":
                return kvList({
                    ASN: d.asn || (d.prefixes && d.prefixes[0] && d.prefixes[0].asn && d.prefixes[0].asn.asn),
                    NAME: d.name || (d.prefixes && d.prefixes[0] && d.prefixes[0].asn && d.prefixes[0].asn.name),
                    COUNTRY: d.country_code || (d.prefixes && d.prefixes[0] && d.prefixes[0].asn && d.prefixes[0].asn.country_code),
                    PREFIXES: d.prefixes ? d.prefixes.length : "—",
                });
            case "dns":
                return Object.entries(d).filter(([, v]) => Array.isArray(v) && v.length)
                    .map(([type, rows]) => `<div class="kv"><span class="k">${type}</span><span class="v">${rows.map(r => escapeHtml(String(r.data || ""))).slice(0, 4).join(" · ")}</span></div>`).join("");
            case "certs":
                return `<div class="kv"><span class="k">CT ROWS</span><span class="v">${d.count}</span></div>` +
                    `<div class="kv-subs">${(d.subs || []).slice(0, 40).map(s => `<a href="https://${s}" target="_blank">${escapeHtml(s)}</a>`).join(" · ")}</div>`;
            case "urlscan":
            ...
            case "wayback":
                ...
                return s ? kvList({ STATUS: s.status, TIMESTAMP: s.timestamp, URL: s.url }) : "<div class='placeholder'>no snapshot</div>";
            case "rdap":
                return kvList({
                    HANDLE: d.handle, NAME: d.name, LDH_NAME: d.ldhName,
                    STATUS: (d.status || []).join(", "),
                    EVENTS: (d.events || []).map(e => `${e.eventAction}:${e.eventDate}`).slice(0, 3).join(" · "),
                });
            case "email":
                return kvList({
                    DOMAIN: d.domain,
                    GRAVATAR: d.gravatar_exists ? `<img src="${d.gravatar_url}&s=48" style="height:24px;vertical-align:middle;border-radius:3px">` : "—",
                    HASH: d.gravatar_hash,
                    MX: (d.mx || []).map(r => r.data).join(" · "),
                    DISPOSABLE: d.disposable ? "YES" : "no",
                });
            case "username":
                return (d || []).slice(0, 80).map(s =>
                    `<a class="user-site" href="${s.url}" target="_blank" title="${escapeHtml(s.category || "")}">${escapeHtml(s.name)}</a>`
                ).join(" ");
            case "github":
                if (!d.user) return "<div class='placeholder'>user not found</div>";
                return kvList({
                    LOGIN: d.user.login, NAME: d.user.name, BIO: d.user.bio,
                    LOCATION: d.user.location, COMPANY: d.user.company, EMAIL: d.user.email,
                    BLOG: d.user.blog, TWITTER: d.user.twitter_username,
                    REPOS: d.user.public_repos, FOLLOWERS: d.user.followers, CREATED: d.user.created_at,
    ...
    function kvList(obj) {
        return Object.entries(obj).filter(([, v]) => v != null && v !== "")
            .map(([k, v]) => `<div class="kv"><span class="k">${k}</span><span class="v">${typeof v === "string" && v.startsWith("<img") ? v : escapeHtml(String(v))}</span></div>`).join("");
    }
    function escapeHtml(s) { return String(s).replace(/[&<>]/g, c => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;" }[c])); }

    $("dos-go") && $("dos-go").addEventListener("click", runSweep);
    dosIn && dosIn.addEventListener("keydown", e => { if (e.key === "Enter") runSweep(); });

    $("dos-trace") && $("dos-trace").addEventListener("click", async () => {
        const q = dosIn.value.trim();
        if (!q) return;
        feed("warn", `TRACE :: ${q}`);
        await GI.traceroute(q);
    });
    $("dos-export-json") && $("dos-export-json").addEventListener("click", () => {
        if (!lastReport) { feed("err", "EXPORT :: run a sweep first"); return; }
        GI.exportJSON(lastReport, `gideon-${lastReport.classification.kind}-${Date.now()}.json`);
    });
    $("dos-export-md") && $("dos-export-md").addEventListener("click", () => {
        if (!lastReport) { feed("err", "EXPORT :: run a sweep first"); return; }
        GI.exportMarkdown(lastReport, `gideon-${lastReport.classification.kind}-${Date.now()}.md`);
    });
    ...
    dnsIn && dnsIn.addEventListener("keydown", e => { if (e.key === "Enter") runDns(); });
    ...
    async function runUser() {
        ...
        const [sites, gh] = await Promise.all([
            GI.usernameLinks(u),
            GI.github(u),
        ]);
        userOut.innerHTML = `
      ...
      <details open><summary><span class="k">WHATSMYNAME</span> <span class="v">${sites.length} sites to probe</span></summary>${renderModule("username", sites)}</details>`;
        feed("ok", `USERNAME :: ${u} → ${sites.length} candidate sites`);
    }
    $("user-go") && $("user-go").addEventListener("click", runUser);
    userIn && userIn.addEventListener("keydown", e => { if (e.key === "Enter") runUser(); });
    ...
    async function runEmail() {
        ...
        feed(d.gravatar_exists ? "ok" : "warn", `EMAIL :: ${e} gravatar=${d.gravatar_exists}`);
    }
    $("em-go") && $("em-go").addEventListener("click", runEmail);
    emIn && emIn.addEventListener("keydown", e => { if (e.key === "Enter") runEmail(); });

    $("pw-go") && $("pw-go").addEventListener("click", async () => {
        const pw = $("pw-input").value;
        if (!pw) return;
        feed("warn", "HIBP :: checking password k-anonymity");
        const r = await GI.pwnedPassword(pw);
        const existing = emOut.innerHTML.includes("placeholder") ? "" : emOut.innerHTML;
        ...
        feed(r.pwned ? "err" : "ok", `HIBP :: password ${r.pwned ? "PWNED × " + r.count : "clean"}`);
    });

    // ---------- Satellites toggle ----------
    const btnSats = $("btn-sats");
    let satsOn = false;
    btnSats && btnSats.addEventListener("click", async () => {
        if (!satsOn) {
            satsOn = true;
    ...
    window.addEventListener("load", () => {
        setTimeout(() => {
            if (GI.replay && GI.replay.all().length) {
                feed("ok", `REPLAY :: ${GI.replay.all().length} past hits available — window.GI.replay.play()`);
            }
        }, 2500);
    });
