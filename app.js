...
Cesium.Ion.defaultAccessToken = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJqdGkiOiJhNmVmNmZiNS0zNTFmLTQ4ODAtYWVhNi1jODk1OTVkOWM1ZDAiLCJpZCI6NDAzNDI5LCJpYXQiOjE3NzM0NjYwMzZ9.Y8p7zOGZEboTqDah5850lRahmOwYMg5kmz8-MDGABnQ";
...
(async () => {
    try {
        ...
        feed("ok", "3D-TILES :: Google Photorealistic 3D Tiles loaded (cities worldwide)");
    } catch (e) {
        console.warn("Google 3D Tiles failed, falling back to OSM Buildings:", e);
        feed("warn", `3D-TILES :: Google photorealistic failed (${e.message}), using OSM Buildings`);
        try {
...
async function loadUsStateLines() {
    ...
    for (const src of sources) {
        try {
            ...
            const polylineCount = geoFeaturesToPolylineEntities(geo, stroke, statesDataSource);
            viewer.dataSources.add(statesDataSource);
            feed("ok", `STATES :: ${polylineCount} polylines rendered (${src.name}, ${geo.features.length} regions)`);
            return;
        } catch (e) {
...
document.getElementById("btn-roads").addEventListener("click", async (e) => {
    ...
    if (on) {
        if (roadsLayer) { viewer.imageryLayers.remove(roadsLayer, false); roadsLayer = null; }
...
scene.skyBox = new Cesium.SkyBox({
    ...
    sources: {
        positiveX: "https://cesium.com/public/SandcastleSampleData/SkyBox/tycho2t3_80_px.jpg",
        negativeX: "https://cesium.com/public/SandcastleSampleData/SkyBox/tycho2t3_80_mx.jpg",
        positiveY: "https://cesium.com/public/SandcastleSampleData/SkyBox/tycho2t3_80_py.jpg",
        negativeY: "https://cesium.com/public/SandcastleSampleData/SkyBox/tycho2t3_80_my.jpg",
        positiveZ: "https://cesium.com/public/SandcastleSampleData/SkyBox/tycho2t3_80_pz.jpg",
        negativeZ: "https://cesium.com/public/SandcastleSampleData/SkyBox/tycho2t3_80_mz.jpg",
    },
});
...
const PLANETS = [
    ...
    { name: "Mercury", a: 0.38709927, e: 0.20563593, i: 7.00497902, O: 48.33076593, w: 77.45779628, L: 252.25032350, dL: 149472.67411175, color: "#ffcc88", size: 8 },
    { name: "Venus", a: 0.72333566, e: 0.00677672, i: 3.39467605, O: 76.67984255, w: 131.60246718, L: 181.97909950, dL: 58517.81538729, color: "#ffeebb", size: 10 },
    { name: "Mars", a: 1.52371034, e: 0.09339410, i: 1.84969142, O: 49.55953891, w: -23.94362959, L: -4.55343205, dL: 19140.30268499, color: "#ff7a5b", size: 9 },
    { name: "Jupiter", a: 5.20288700, e: 0.04838624, i: 1.30439695, O: 100.47390909, w: 14.72847983, L: 34.39644051, dL: 3034.74612775, color: "#ffd27f", size: 14 },
    { name: "Saturn", a: 9.53667594, e: 0.05386179, i: 2.48599187, O: 113.66242448, w: 92.59887831, L: 49.95424423, dL: 1222.49362201, color: "#ffe8a8", size: 13 },
    { name: "Uranus", a: 19.18916464, e: 0.04725744, i: 0.77263783, O: 74.01692503, w: 170.95427630, L: 313.23810451, dL: 428.48202785, color: "#bfeeff", size: 10 },
    { name: "Neptune", a: 30.06992276, e: 0.00859048, i: 1.77004347, O: 131.78422574, w: 44.96476227, L: -55.12002969, dL: 218.45945325, color: "#7fb9ff", size: 10 },
];
...
function keplerianToHeliocentric(p, jd) {
    ...
    const xe = (cosw * cosO - sinw * sinO * cosi) * xp + (-sinw * cosO - cosw * sinO * cosi) * yp;
    const ye = (cosw * sinO + sinw * cosO * cosi) * xp + (-sinw * sinO + cosw * cosO * cosi) * yp;
...
const EARTH = { a: 1.00000261, e: 0.01671123, i: -0.00001531, O: 0, w: 102.93768193, L: 100.46457166, dL: 35999.37244981 };
...
PLANETS.forEach((p) => {
    viewer.entities.add({
        ...
        position: new Cesium.CallbackProperty(() => {
            ...
            const jd = Cesium.JulianDate.toDate(Cesium.JulianDate.fromDate(now)) && julianDate(now);
            ...
            const rot = Cesium.Transforms.computeIcrfToFixedMatrix(viewer.clock.currentTime);
            if (rot) return Cesium.Matrix3.multiplyByVector(rot, dir, new Cesium.Cartesian3());
            return dir;
        }, false),
        billboard: {
...
document.querySelectorAll(".tool[data-view]").forEach(btn => {
    btn.addEventListener("click", () => {
        ...
        document.querySelectorAll(".tool[data-view]").forEach(b => b.classList.remove("active"));
...
const HOTSPOTS = [
    ...
    { name: "Chernobyl", lat: 51.3890, lon: 30.0990 },
    { name: "Pyongyang", lat: 39.0392, lon: 125.7625 },
    { name: "DEF CON / Vegas", lat: 36.1100, lon: -115.1733 },
    { name: "Kremlin", lat: 55.7520, lon: 37.6175 },
    { name: "CERN", lat: 46.2333, lon: 6.0500 },
...
document.getElementById("btn-clear").addEventListener("click", () => {
    ...
    targets.forEach(t => {
        try { viewer.entities.removeById(t.id); } catch { }
        (t.extras || []).forEach(eid => { try { viewer.entities.removeById(eid); } catch { } });
    });
    targets.length = 0;
    renderTargets();
...
document.querySelectorAll(".tab").forEach(tab => {
    tab.addEventListener("click", () => {
        ...
        document.querySelector(`.tab-body[data-body="${tab.dataset.tab}"]`).classList.add("active");
    });
});
...
function pinTarget({ label, lat, lon, meta, danger = false }) {
    ...
    const t = { id: entity.id, label, lat, lon, meta, danger, ts: Date.now(), extras: [tower.id, beam.id] };
    targets.push(t);
    renderTargets();
    feed(danger ? "err" : "ok", `PIN :: ${label} @ ${lat.toFixed(3)}, ${lon.toFixed(3)}`);
    return t;
}

// Build a 3D parabolic arc between two lat/lon points (peaks at `peakHeight`).
function arcPositions(lon1, lat1, lon2, lat2, peakHeight = 400_000, steps = 64) {
...
async function geoLookup(query) {
    ...
    try {
        ...
        feed("ok", `GEO-IP :: ${data.ip} → ${data.city}, ${data.country_code} [${data.provider}]`);
    } catch (err) {
...
function renderGeo(d) {
    ...
    ipResult.innerHTML = rows.map(([k, v]) =>
...
ipInput.addEventListener("keydown", e => { if (e.key === "Enter") document.getElementById("ip-go").click(); });
document.getElementById("ip-self").addEventListener("click", () => geoLookup(""));

// ---------- LINK-TRACE (GideonIntel) ----------
// Generates a GideonIntel short link. In this local demo we intercept
// the click via a URL hash so we can simulate a geo-IP pinned target.
const linkOut = document.getElementById("link-out");
document.getElementById("link-gen").addEventListener("click", () => {
    const target = document.getElementById("link-url").value.trim() || "https://youtube.com";
    const tag = document.getElementById("link-tag").value.trim() || "untagged";
    const slug = Math.random().toString(36).slice(2, 8);
    const tracked = `https://gideonintel.io/${slug}`;
    const localSim = `${location.origin}${location.pathname}#track=${slug}&to=${encodeURIComponent(target)}&tag=${encodeURIComponent(tag)}`;
    linkOut.innerHTML = `
    <div class="kv"><span class="k">INTEL LINK</span><span class="v">${tracked}</span></div>
    <div class="kv"><span class="k">CAMPAIGN</span><span class="v">${tag}</span></div>
    <div class="kv"><span class="k">REDIRECT</span><span class="v">${target}</span></div>
    <div class="kv"><span class="k">LOCAL SIM</span><span class="v"><a href="${localSim}" target="_blank">${localSim}</a></span></div>
  `;
    feed("warn", `LINK :: generated /${slug} → ${target}`);
    // Simulate an incoming click after a short delay
    setTimeout(() => simulateClick(slug, tag), 4000 + Math.random() * 4000);
});

async function simulateClick(slug, tag) {
    // Use self-geoip as the "victim" for the demo (lab only)
    try {
        const d = await fetchGeo("");
        feed("err", `HIT :: /${slug} clicked from ${d.city}, ${d.country_code}`);
        pinTarget({
            label: `/${slug}`,
            lat: d.lat, lon: d.lon,
            meta: `${tag} · ${d.org || "?"}`,
        ...
        // Persist to GideonIntel replay history (survives reload)
...
(function handleTrackHash() {
    ...
    if (to) setTimeout(() => { /* would redirect in real GideonIntel */ }, 1200);
})();
...
removeHandler.setInputAction((click) => {
    ...
    const tIdx = targets.findIndex(t =>
        t.id === pickedId || (t.extras && t.extras.includes(pickedId))
    );
    if (tIdx === -1) return;
...
console.log("%cGideonsEarth ready — double-click the globe to drop a pin.",
...
async function loadIonAsset(asset) {
    ...
    try {
        if (asset.type === "3DTILES") {
            ...
            if (stageCount === 0) {
                autoRotate = false;
                try { await viewer.zoomTo(ts); } catch { /* ignore */ }
            }
            stageCount++;
            ...
            feed("ok", `MODEL :: GLB #${asset.id} "${asset.name}" placed @ ${lat.toFixed(3)}, ${lon.toFixed(3)}`);
            stageCount++;
...
async function autoLoadAllIonAssets() {
    try {
        ...
        const mine = assets.filter(a => !COMMUNITY_IDS.has(a.id) && a.status === "COMPLETE");
        feed("ok", `ION :: found ${mine.length} asset(s) in your account`);
        if (mine.length === 0) {
            feed("warn", "ION :: upload models at https://ion.cesium.com/assets to render them here");
            return;
        }
...
document.getElementById("asset-refresh").addEventListener("click", autoLoadAllIonAssets);
...
function addUploadRow(name) {
    ...
    return {
        ...
        setPct: (p) => { row.querySelector(".pct").textContent = `${p}%`; },
        setStatus: (s, cls) => {
...
async function uploadToIon(file) {
    ...
    try {
        ...
        if (!createRes.ok) throw new Error(`create asset ${createRes.status}: ${await createRes.text()}`);
        ...
        const onDoneRes = await fetch(onComplete.url, {
            ...
            headers: { ...onComplete.fields ? { "Content-Type": "application/json" } : {}, "Authorization": `Bearer ${token}` },
...
async function enterWalkMode(lon, lat, pickedHeight) {
    ...
    walk.avatarEntity = viewer.entities.add({
        position: new Cesium.CallbackProperty(() => {
            ...
            return Cesium.Cartesian3.fromRadians(p.longitude, p.latitude, p.height - walk.eyeHeight + 0.9);
        }, false),
        orientation: new Cesium.CallbackProperty(() => {
            const hpr = new Cesium.HeadingPitchRoll(walk.heading, 0, 0);
            const pos = Cesium.Cartesian3.fromRadians(walk.position.longitude, walk.position.latitude, walk.position.height);
    ...
    viewer.canvas.requestPointerLock = viewer.canvas.requestPointerLock || viewer.canvas.mozRequestPointerLock;
...
document.addEventListener("mousemove", e => {
    ...
    walk.pitch = Math.max(-Math.PI / 2 + 0.05, Math.min(Math.PI / 2 - 0.05, walk.pitch));
});
    ...
    try {
        ...
        if (scene.clampToHeightSupported) {
            ...
            const testPos = Cesium.Cartesian3.fromRadians(
                walk.position.longitude, walk.position.latitude, 10_000
            );
            const clamped = await scene.clampToHeightMostDetailed([testPos], exclude, 1.0);
            ...
            const [c] = await Cesium.sampleTerrainMostDetailed(
                viewer.terrainProvider,
                [Cesium.Cartographic.fromRadians(walk.position.longitude, walk.position.latitude)]
            );
            if (Number.isFinite(c.height)) walk.groundHeight = c.height;
        }
...
viewer.clock.onTick.addEventListener(() => {
    ...
    const sinH = Math.sin(walk.heading), cosH = Math.cos(walk.heading);
    ...
    walk.position.latitude += (north * moveSpeed * dt) / metersPerDegLat * Math.PI / 180;
    walk.position.longitude += (east * moveSpeed * dt) / metersPerDegLon * Math.PI / 180;
    ...
    walkHdgEl.textContent = `${Math.round((Cesium.Math.toDegrees(walk.heading) % 360 + 360) % 360).toString().padStart(3, "0")}°`;
});
...
async function geocodeAndFly(query) {
    ...
    try {
        ...
        feed("ok", `GEOCODE :: "${name.split(",").slice(0, 2).join(",").trim()}" → ${lat.toFixed(4)}, ${lon.toFixed(4)}`);
...
(function spawnSatellite() {
    ...
    for (let i = 0; i <= 128; i++) {
        ...
        const lat = Math.asin(Math.sin(INCLINATION * Math.PI / 180) * Math.sin(theta)) * 180 / Math.PI;
    ...
    const panelOffset = (sign) => new Cesium.CallbackProperty(() => {
        ...
        return Cesium.Cartesian3.add(
            basePos,
            Cesium.Cartesian3.multiplyByScalar(east, sign * 180_000, new Cesium.Cartesian3()),
